...
# PARAMETERS
# paths
INPUT=/var/data/store-lidarhd/developpement/ctview/las/data0b
OUTPUT=/var/data/store-lidarhd/developpement/ctview/1_tests_local/test0b
# number of file in INPUT
NB_FILE_EXPECTED="1"
NB_FILE_EXPECTED_FOR_DENSITY="0"
# folders
...
if [ "$nb_file_folder1" = "$NB_FILE_EXPECTED_FOR_DENSITY" ]; then
    echo "OK : density : number of files : $NB_FILE_EXPECTED_FOR_DENSITY expected $nb_file_folder1 obtained -> no ground point."
else
    echo "ERROR : density : number of files : $NB_FILE_EXPECTED_FOR_DENSITY expected $nb_file_folder1 obtained -> no ground point."
fi
